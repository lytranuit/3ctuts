...
class Ion_auth_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        $query = $this->db->get_where($this->tables['groups'], array('name' => $this->config->item('default_group', 'ion_auth'), 'deleted' => 0), 1)->row();
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login,img_user')
                ->where($this->identity_column, $identity)
                ->where('deleted', 0)
                ->limit(1)
                ->order_by('id', 'desc')
    ...
    public function get_users_groups($id = FALSE) {
        ...
        return $this->db->select($this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description')
                        ...
                        ->where($this->tables['groups'] . '.' . 'deleted', 0)
                        ->where($this->tables['users_groups'] . '.' . 'deleted', 0)
    ...
    public function delete_user($id) {
        ...
        $this->db->update($this->tables['users'], array('deleted' => 1), array('id' => $id));
    ...
    public function delete_group($group_id = FALSE) {
        ...
        $this->db->update($this->tables['users_groups'], array('deleted' => 1), array($this->join['groups'] => $group_id));
        // remove the group itself

        $this->db->update($this->tables['groups'], array('deleted' => 1), array('id' => $group_id));
